...
test_that("stash() stashes internals to the modulr environment", {
  ...
  stash()
  expect_equal(length(modulr_env$stash), 1)
  stashed <- modulr_env$stash[[1]]
  expect_equal(stashed$register, modulr_env$register)
  expect_equal(stashed$.Last.name, modulr_env$.Last.name) # Exclude Linting
  expect_equal(stashed$config, modulr_env$config)
