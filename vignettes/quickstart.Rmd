---
title: "Modulr Quick-start Guide"
...
vignette: >
  %\VignetteIndexEntry{Modulr Quick-start Guide}
...
graph_dependencies() %>%
  html_print %>%
  webshot(file = "quickstart-foobar.png", selector = ".sankeyNetwork", delay = 2) %>%
  resize("730x") %>%
  shrink()
```
...
To see the essence of what is happening, we decrease the verbosity of modulr and define a module `foo` that requires
`timestamp`, defined as a singleton:
